...
class hQuery extends HTML_Parser {
    ...
    public static function sendRequest(RequestInterface $request, $client=NULL) {
        // if ( !class_exists('system\lib\hquery\HttpClientDiscovery') ) {
        //     throw new Exception('system\lib\hquery\HttpClientDiscovery not found. Install a client (eg. `require php-http/curl-client`)');
        // }
        if ( !isset($client) ) {
     ...
     *  @param \system\lib\hquery\Node  $ctx  - OPTIONAL the context where to search. If omitted, $this is used.
     *
     *  @return \system\lib\hquery\Element collection of matched elements or NULL
     */
...
// ------------------------------------------------------------------------
// PSR-0 alias
class_exists('system\lib\hquery', false) ;
